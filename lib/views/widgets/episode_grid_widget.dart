...
class _EpisodeGridWidgetState extends State<EpisodeGridWidget> {
  ...
  List<_EpisodeRange> get _ranges {
    ...
    if (totalEpisodes <= widget.episodesPerRange) {
      ...
      ranges.add(_EpisodeRange(
        start: 1,
        end: totalEpisodes,
        label: '1-$totalEpisodes',
      ));
    } else {
      // Multiple ranges
  ...
  List<EpisodeModel> get _currentRangeEpisodes {
    ...
    return widget.episodes
        .where((ep) => 
            (ep.number ?? 0) >= range.start && 
            (ep.number ?? 0) <= range.end)
        .toList();
  }
    ...
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final isWatched = widget.lastWatchedEpisode != null && 
      ...
      child: ListTile(
        ...
        trailing: Row(
          ...
          children: [
            ...
            if (isLastWatched)
              const Icon(Icons.play_circle_filled, color: OnePieceTheme.strawHatRed),
            if (!isLastWatched)
              Icon(
